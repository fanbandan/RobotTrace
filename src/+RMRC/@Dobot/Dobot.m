classdef Dobot < RMRC.Robot
    %Dobot Class for controlling the simulation robot and calculating kinematics.
    ...
    methods
        ...
        function self = Dobot()
            %Dobot creates dobot and calls base class constructor.
            self@RMRC.Robot();
        ...
        function GetRobot(self)
            %GetRobot creates and return Dobot Magician on linear rail robot model
            pause(0.001);
            name = ['Linear Dobot'];
        ...
        function PlotAndColourRobot(self, workspace, q)
            %PlotAndColourRobot OBSOLETE
            %   No model avaliable for dobot magician.
            PlotRobot(self, workspace, q)
        end

        function PlotRobot(self, workspace, q)
            %PlotRobot tranforms from real q values to model values
            %   workspace - The X,Y,Z axis limits
            %   q - Expects a 1x5 q vector
            if nargin <= 2
        ...
        function Animate(self, q)
            %Animate plots robot over a series of q values.
            %   PlotRobot must be called once before this method.
        ...
        function q = GetPos(self)
            %GetPos returns the simulation model's current joint values.
            q = self.Model.getpos();
            q(4) = q(4) - pi / 2 + q(3);
            q = [q(1:4), q(6)];
        end
        ...
        function T = fkine(self, q)
            %fkine calculates robot end effector pose for q values.
            %   q - Expects a nx5 q vector, different to q
            %       values used for Peter Corke's serial link.
            T = NaN(4, 4, size(q, 1));
        ...
        function [x, y, z] = LinkKinematics(self, q)
            %LinkKinematics returns the x,y,z of the link joint determined by q.
            %   q - Expects a 1x(1 to 5) q vector, different to q
            %       values used for Peter Corke's serial link.
            switch (size(q, 2))
                case 1
                    x = q(:, 1);
        ...
        function [qstar, error, exitflag] = ikcon(self, T, q0)
            %ikcon calculates q values to reach the desired end effector pose.
            %   T - The desired end effector pose.
            %   q0 - Optional, the start joint state.
        ...
        function J = jacob(self, q)
            %jacob calculates the jacobian for the dobot at given joint state.
            %   q - The joint state.
        ...
        function [valid, q] = verifyQ(self, q)
            %verifyQ checks to see if joint state is within the dobot joint limits.
            valid = true;
